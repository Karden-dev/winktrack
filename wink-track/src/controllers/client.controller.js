// karden-dev/winktrack/src/controllers/client.controller.js
...
const PRICE_PER_KM = 100;   // Mise à jour : Prix au kilomètre (100 FCFA)
...
exports.createOrder = async (req, res) => {
    try {
        ...
        // --- SÉCURITÉ CRITIQUE : Validation de pickupLoc pour éviter le crash TypeError ---
        if (!pickupLoc || pickupLoc.lat === undefined || pickupLoc.lng === undefined) {
            console.error("❌ Erreur : pickupLoc est nul ou incomplet.");
            return res.status(400).json({ 
                success: false, 
                error: "La position de départ est requise. Veuillez sélectionner un point sur la carte." 
            });
        }

        // Gestion Shadow Account
...
exports.getHistory = async (req, res) => {
    try {
        ...
        const sql = `
            SELECT o.id, o.pickup_desc, o.created_at, o.estimated_price, o.status, o.distance_km
            FROM orders o 
            WHERE o.sender_id = ? 
        ...
        const history = rows.map(row => ({
            ...
            dest: row.pickup_desc || "Destination Inconnue",
...
exports.getProfile = async (req, res) => {
    try {
        ...
        res.json({
            ...
            paymentNumber: client.default_payment_number || "", 
            balance: 0 
        });
